 ...
 *$Id: c1c59568d39fe359f82aea423eab865c6e254372 $
 *
...
#include "common.h"
	ID3DXEffect* d3dps; //pixel shader 
/*****************************************************************************
 * Module descriptor
 *****************************************************************************/
...
static int Open(vlc_object_t *object)
{
    ...
    if (Direct3DCreate(vd)) {
		...
		msg_Err(vd, "Direct3D initialized");
    ...
    if (sys->use_desktop && vd->cfg->is_fullscreen)
		...
		sys->hd3d9_dll = LoadLibrary(TEXT("D3dx9d_43.dll"));
    if (!sys->hd3d9_dll) {
        msg_Err(vd, "cannot load D3dx9d_43.dll");
    }
	else
		msg_Err(vd, "loaded D3dx9d_43.dll");
    ...
    OurD3DXCreateEffectFromFileA =
        (void *)GetProcAddress(sys->hd3d9_dll, TEXT("D3DXCreateEffectFromFileA"));
    if (!OurD3DXCreateEffectFromFileA) {
        msg_Err(vd, "Cannot locate reference to D3DXCreateEffectFromFileA ABI in DLL");
    }
	else
		msg_Err(vd, "D3DXCreateEffectFromFileA found");


	DWORD dwShaderFlags = D3DXFX_NOT_CLONEABLE;
	HRESULT hr = OurD3DXCreateEffectFromFileA( sys->d3ddev, psFile, NULL, NULL, dwShaderFlags, NULL, &d3dps, NULL );
	if (FAILED(hr)) {
       msg_Err(vd, "D3DXCreateEffectFromFileA Error with file \"%s\" (hr=0x%lX)",psFile, hr);
       //return VLC_EGENERIC;
    }
	else
		msg_Err(vd, "Pixel shader file \"%s\" loaded",psFile);
...
static void Direct3DRenderScene(vout_display_t *vd, LPDIRECT3DSURFACE9 surface)
{
	...
	/*
		...
		HRESULT (WINAPI * MySetTechnique)(
        ID3DXEffect               effect,
        D3DXHANDLE hTechnique);

    MySetTechnique =
        (void *)GetProcAddress(sys->hd3d9_dll, TEXT("ID3DXEffect::SetTechnique"));
    if (!MySetTechnique) {
        msg_Err(vd, "Cannot locate reference to SetTechnique ABI in DLL");
    }
	else
		msg_Err(vd, "SetTechnique found");
	*/
	//ID3DXEffect_SetTechnique(d3dps,"PostProcess");
	d3dps->lpVtbl->SetTechnique(d3dps, "PostProcess" );
	if (FAILED(hr)) {
       msg_Err(vd, "SetTechnique failed (hr=0x%lX)", hr);
       //return VLC_EGENERIC;
    }
	//else
	//	msg_Err(vd, "SetTech Success");


    hr = d3dps->lpVtbl->Begin(d3dps, &uPasses, 0 );
	if (FAILED(hr))
       msg_Err(vd, "Begin failed (hr=0x%lX)", hr);

	for( UINT uPass = 0; uPass < uPasses; ++uPass )
    {
		hr = d3dps->lpVtbl->BeginPass(d3dps, uPass);
		if (FAILED(hr))
	...
	d3dps->SetTechnique( "PostProcess" );
	*/
    ...
    if (FAILED(hr)) {
		...
		hr = d3dps->lpVtbl->EndPass(d3dps);
		if (FAILED(hr))
			msg_Err(vd, "EndPass failed (hr=0x%lX)", hr);

	}

	hr = d3dps->lpVtbl->End(d3dps);
	if (FAILED(hr))
